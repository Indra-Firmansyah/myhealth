...
class SalonBookingBackend(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('proses', 'Proses'), ('batal', 'Batal')], default="draft")
    time = fields.Datetime(string="Date")
    phone = fields.Char(string="Phone")
    email = fields.Char(string="E-Mail")
    bpjs = fields.Char(string="NO BPJS")
    ...
    def booking_approve(self):
        ...
        template = self.env.ref('myhealth.salon_email_template_approved')
        self.env['mail.template'].browse(template.id).send_mail(self.id)
        self.state = "approved"

    @api.multi
    def booking_reject(self):
        template = self.env.ref('myhealth.salon_email_template_rejected')
        self.env['mail.template'].browse(template.id).send_mail(self.id)
        self.state = "rejected"
